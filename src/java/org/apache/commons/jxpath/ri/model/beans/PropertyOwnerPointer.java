...
public abstract class PropertyOwnerPointer extends NodePointer {
	...
	private Object value = PropertyOwnerPointer.UNINITIALIZED;
		...
		if (test instanceof NodeNameTest) {
			...
			if (isValidProperty(testName)) {
				return createNodeIterator(nodeNameTest.isWildcard() ? null
						: PropertyIdentifier.fromQName(namespaceUri, testName, isAttribute()), reverse, startWith);
			}
			return null;
		}
	 ...
	 * @return NodeIterator
	 */
	public NodeIterator createNodeIterator(final PropertyIdentifier property, final boolean reverse,
	                                       final NodePointer startWith) {
	...
	public void setIndex(final int index) {
		if (this.index != index) {
			...
			value = PropertyOwnerPointer.UNINITIALIZED;
		}
	}

	public Object getImmediateNode() {
		if (value == PropertyOwnerPointer.UNINITIALIZED) {
			value = index == NodePointer.WHOLE_COLLECTION ? ValueUtils.getValue(getBaseValue())
	...
	public String getNamespaceURI() {
		return this.getPropertyPointer().getPropertyName().getNamespaceUri();
	}
